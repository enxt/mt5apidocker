...
#: How many IPC failures before the process exits for supervisor to restart.
#: Not 1: on a fresh container the terminal spends its first minutes starting
#: up and logging in, and every attempt in that window times out. Exiting on
#: the first one only restarted the API (which takes ~30s under Wine) in a
#: loop that never let the terminal finish.
MAX_IPC_RETRIES = 5
...
class MT5Connector:
    ...
    Waits for run-server.sh to create LOGIN_MARKER (terminal started and
    logging itself in from startup.ini), then calls mt5.initialize() in a
    background thread to establish the IPC pipe without blocking uvicorn.
    ...
    def _do_initialize(self):
        ...
        try:
            ...
            # Try without credentials first — the terminal logs itself in
            # from startup.ini, and passing credentials again would trigger
            # an "account changed" event.
            logger.info(f"MT5 initialization started (login={login}, server={server})...")
            success = mt5.initialize(MT5_PATH, portable=True)

            if not success:
                # Fallback: pass credentials, for when the terminal has not
                # finished (or could not do) its own login.
            ...
            if success:
                ...
                # startup.ini and common.ini enable it)
